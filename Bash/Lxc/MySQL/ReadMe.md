# <img src="../../../Assets/lxd-lxc_logo.png" width=35 alt="Bash Scripts Collection">  Installing and managing **MySQL** in **LXC/LXD** containers

[![LXC/LXD](https://custom-icon-badges.demolab.com/badge/LXC_LXD-Containers-607078?style=flat&logo=lxd-lxc_logo&logoColor=grey&logoSize=auto&labelColor=grey)](https://documentation.ubuntu.com/lxd/stable-5.21/)
[![Bash](https://img.shields.io/badge/GNU%20Bash-4EAA25?style=flat&logo=gnubash&logoColor=white&logoSize=auto&labelColor=black)](https://www.gnu.org/software/bash/)
[![License: MIT](https://img.shields.io/badge/License-MIT-green.svg)](https://opensource.org/licenses/MIT)

Some examples how to install and configure **MySQL** inside **LXC/LXD** conatainers on **Debian/Ubuntu**.  

## 📂 Description

- 📂 [Postgre](./Postgre/)
  </p>Installing or managing Postgre SQL on Debian/Ubuntu LXC/LXD container
- 📂 [MySQL](./MySQL/)
  </p>Installing or managing Postgre SQL on Debian/Ubuntu LXC/LXD container
- 📂 [MongoDb](./Mongo/)
  </p> Installing and managing MongoDB on Debian/Ubuntu LXC/LXD container

---

🔙 [back to 📂 Scripting](../)
