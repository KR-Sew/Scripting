...
fix_certbot_compatibility() {
    echo "Ensuring Certbot compatibility with custom NGINX path..."

    # Create /etc/nginx if missing
    if [ ! -d /etc/nginx ]; then
    ...
    if [ ! -L /etc/nginx/nginx.conf ]; then
        ...
        sudo ln -s "$NGINX_CONF_DIR"/* /etc/nginx/
    fi

    # Ensure nginx binary in PATH
    if ! command -v nginx >/dev/null 2>&1; then
        sudo ln -sf "$NGINX_BIN_PATH" /usr/bin/nginx
    fi

    echo "Certbot compatibility layer installed."
}
